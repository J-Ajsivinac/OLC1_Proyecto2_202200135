...
export class InitMatrix extends Instruction {
        ...
        if (this.values) {
            ...
            for (let i = 0; i < this.values.length; i++) {
                ...
                for (let j = 0; j < this.values[i].length; j++) {
                    ...
                    vals.push(this.values[i][j])
                }
                // this.values[i] = values_t
                // console.log("values_t", vals)
                values_t.push({ value: vals, type: Types.ARRAY })
            }
            // console.log("Esto es values", values_t[0].value) // obtener el objeto
            // console.log("values", values_t)
            env.saveArray(this.id, this.type, values_t, this.line, this.column)
        } else {
            let length: ReturnType = this.size.execute(env)
            console.log("values 2.0", this.arrayByLength(length.value, this.type, env))
    ...
    private arrayByLength(length: number, type: Types, env: Environment): Array<any> {
        let l: ReturnType[] = new Array(length)
        for (let i = 0; i < length; i++) {
            let length2: number = this.size2.execute(env).value
            // l[i] = new Array(length2)
            let vals: ReturnType[] = []
            for (let j = 0; j < length2; j++) {
                vals.push({ value: getValueDefaultArray(type, this.type), type: this.type })
            }
            l[i] = { value: vals, type: Types.ARRAY }
        }
        return l;
    }
